...
std::vector<Edge> readEdgesFromBinary(){
    ...
    file.open("../binaries/layer-5.raw", std::ios::in | std::ios::binary);
...
void writeToAsciiFile(std::vector<int>peelValues){

    // writing to ascii file
    std::ofstream myfile;
    std::string name( "../Ascii_files/v_p_in_ascii_file.csv" );
    std::cout<< "Creating a Peel value: "<<name<<std::endl;
    myfile.open (name);
    for(int i =1;i<peelValues.size();i++){
        
        myfile <<revMap[i] <<","<<peelValues[i]<<"\n";
    }
    std::cout<< "Finished creating a Peel value: "<<name<<std::endl;
}


void writeToBinaryFile(std::vector<int> peelValues){

    std::fstream my_binary_file;;
    my_binary_file.open("../binaries/v_p.raw", std::ios::out | std::ios::binary);
    std::cout<< "Writing v,p to binary file: "<<std::endl;
    int count = 0;
    for(int i =1;i<=peelValues.size();i++){
        int a = revMap[i], b = peelValues[i];
...
int main()
{
    ...
    writeToAsciiFile(peelValues);
    writeToBinaryFile(peelValues);
    // Get ending timepoint
